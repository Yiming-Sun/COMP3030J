...
condition_dict = {
    1:_("appoint success(1)"),
    2:_("diagnosing(2)"),
    3:_("reserve operation(3)"),
    4:_("operating(4)"),
    5:_("finish operation(5)"),
    6:_("pay for medicine(6)"),
    7:_("out(7)")
}
...
def Language():
    ...
    if languageame == "en":
        ...
        print("en")
        # return redirect(url_for('index'))
    return render_template('CGL.html', day=day)
...
def logout():
    if session['USERTYPE'] == 'customer':
            print("i am customers")
            cus = Customer.query.filter(session['USERNAME'] == Customer.username).first()
            print(cus)
            if cus:
                cus.log_con = 0
                print("success")
            ...
            emp = Employee.query.filter(session['USERNAME'] == Employee.username).first()
            if emp:
                emp.log_con = 0
                db.session.commit()
            ...
            if form.validate_on_submit():
                ...
                if request.values.get("type"):
                    ...
                    for doctor in doctors:
                        name_list[doctor.id] = doctor.nickname
                    #print(name_list)
                    return name_list
        ...
        if request.method == 'POST':
            ...
            #print(data)
                ...
                for d in data:
                    ...
                    doc_list.append(Employee.query.filter(d['doctor'] == Employee.id).first().nickname)
            ...
            for employee in employees:
                doct_list[employee.id] = employee.nickname

            return render_template('My_appointment.html', img=img, title='My Appointment',my_appointments=my_appointments,petT=petT, condition_dict=condition_dict, doct_list=doct_list,time_dict=time_dict)
    else:
        flash("User needs to either login or signup first")
...
def sign_in_c():
    ...
    if form.validate_on_submit():
        ...
        if not user_in_db:
            ...
            return redirect(url_for('sign_in_c'))
        if user_in_db.log_con == 1:
            flash('This account {} is in login status, unable to login'.format(form.username.data))
            return redirect(url_for('sign_in_c'))
        if (check_password_hash(user_in_db.password_hash, form.password.data)):
            session["USERNAME"] = user_in_db.username
            session["USERTYPE"] ='customer'
            session["USERID"] = user_in_db.id
            #user_in_db.log_con = 1
            db.session.commit()
            return redirect(url_for('My_appointment'))
...
def appointment_type(place):
    ...
    session["PLACE"] = place
    return render_template('appointment_type.html', title="Appointment type", img=img)
...
def location_type():
    ...
    return render_template('location_type.html', title="Location type", img=img)
    ...
    if request.method == 'POST':
        ...
        if request.values.get("type") == "username":
            ...
            if form.validate_on_submit():
                ...
                if form.workplace.data == 1:
                    workplace = "Beijing"
                elif form.workplace.data == 2:
                    workplace = "Shanghai"
                else:
                    workplace = "Chengdu"
...
@app.route('/detail')
def detail():
    return render_template('details.html',title='Detail')
